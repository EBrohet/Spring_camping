...
public class EcheanceController {
    ...
    @GetMapping("/{id}")
    public ResponseEntity<Echeance> getEcheanceById(@PathVariable("id") Integer id){
        try {
            HttpHeaders headers = new HttpHeaders();
            headers.set(HttpHeaders.CONTENT_TYPE, "application/json");
    ...
    public ResponseEntity<String> updateEcheance(@RequestBody Echeance echeance){
        try {
            ...
            return new ResponseEntity<>("{message : 'Echéance modifiée'}", headers, HttpStatus.CREATED);
        } catch (Exception e) {
            return new ResponseEntity<>(HttpStatus.INTERNAL_SERVER_ERROR);
        }
    }
        ...
        @DeleteMapping("/{id}")
        public ResponseEntity<String> deleteEcheance(@PathVariable("id") Integer id){
        try {
            HttpHeaders headers = new HttpHeaders();
            headers.set(HttpHeaders.CONTENT_TYPE, "application/json");
            echeanceService.deleteEcheance(id);
            return new ResponseEntity<>("{message : 'Echéance supprimée'}", headers, HttpStatus.OK);
        } catch (Exception e) {
            return new ResponseEntity<>(HttpStatus.INTERNAL_SERVER_ERROR);
        }
    }
